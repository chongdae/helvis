...
{
	unsigned char	key[3];		/* key char followed by '\0' char */
	unsigned char	keyTemp[2];	/* Hangul 1st byte followed by '\0'*/
					/* for v_subst() with Hangul input */
	int	nByteInput;
	...
	else /* interactive version */
	{
		...
		if (from != to && markline(from) == markline(to))
		{
			...
			if (IsHIdx(ptext, markidx(to-1)) == HAN_SECOND)
			{
				change(to - 2, to, " $");
			}
			else
		...
		for (;;)
		{
			...
			if (IsHiBitOn( key[0] ))
			{
				key[1] = getabkey(when, scan, (int)(ptext + markidx(cursor) - scan));
				nByteInput = 2;
			}
			else
			{
				...
				else
				{
					...
					else
					{
						...
						*key = digraph(ptext[markidx(cursor)], *key);
						...
						{
							if ((nByteInput == 1) && IsHiBitOnMark(cursor))
							{ /* An ASC over 1st half of HANGUL */
								change(cursor+1, cursor+2, " ");
								addspace = 1;
							}
							else if ((nByteInput==2) && !IsHiBitOnMark(cursor) && IsHiBitOnMark(cursor+1))
							{ /* an HANGUL over ASC+HANGUL */
...
BreakBreak:;
	...
	if (markidx(cursor) != 0)
	{
		cursor--;
		pfetch(markline(cursor));
		if (IsHIdx(ptext,markidx(cursor)) == HAN_SECOND)
			cursor--;
	}
	redraw(cursor, FALSE);
