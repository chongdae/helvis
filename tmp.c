 ...
 *	kirkenda@cs.pdx.edu
 *
 * Modified by Tom Bodoh 02/10/93 to preserve the protection of previous
 * version of the edited file instead of using the user default protection.
 */
...
# if AMIGA	/* Don't SLASH a device. "Elvis:.exrc" */
		if (filename[-1] != COLON && filename[-1] != SLASH)
# else
		if (filename[-1] != SLASH)
# endif
		{
	...
	int	bang;		/* forced write? */
{
	...
	else
	{
...
#if VMS
		/* On VMS we want to propogate the file protection
		 * of the previous version of the file, if any.
		 * If the stat fails, protection defaults to user default
		 */
		statb.st_mode = 0;
		stat(filename, &statb);
		...
		fd = creat(filename,(statb.st_mode & 511));
#else
	...
	/* close the file */
#if MSDOS
	/* Novell requires a ^Z at the end of the text? */
	if (*o_controlz)
	{
		twrite(fd, "\032", 1);
	}
	...
	close(fd);
#if VMS
        chmod(filename,(unsigned int)(statb.st_mode & 511));
#endif
	return TRUE;
}
