	...
	int	prevkey;/* previous command... if 'c' then exclude whitespace */
{
	...
	while (cnt-- > 0) /* yes, ASSIGNMENT! */
	{
		...
		{
			/* include continuous Hangul chars */
			while ( i && IsHiBitOn(i))
			{
				text++;
				i = *text++;
		...
		else
		{
			...
			while (i && !IsHiBitOn(i) && !isalnum(i) && !isspace(i))
			{
				i = *text++;
			}
		}
	...
	int	cmd;	/* either 'b' or 'B' */
{
	...
	while (cnt-- > 0) /* yes, ASSIGNMENT! */
	{
		...
		else if (AtHangul2(text))
		{
			/* include continuous Hangul */
			while (text >= ptext && AtHangul2(text))
			{
				text -= 2;
			}
		}
		...
		{
			/* include contiguous punctuation */
			while (text >= ptext && !AtHangul2(text) && !isalnum(*text) && !isspace(*text))
			{
				text--;
			}
		}
